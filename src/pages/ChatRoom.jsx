...
const ChatDiv = styled.div`
  ...
  max-width: 60%;
  margin-left: 12px;
  background-color: ${(props) => (props.className === 'you' ? props.theme.color.text.white : props.theme.color.main.green)};
  box-sizing: border-box;
  border: ${(props) => (props.className) === 'you' ? '1px solid props.theme.color.gray.d3' : ''};
  border-radius: ${(props) => (props.className) === 'you' ? '0 10px 10px 10px' : '10px 0 10px 10px'};
  font-size: 14px;
  font-weight: 400;
  line-height: 18px;
  padding: 12px;
  text-align: ${(props) => (props.className) === 'me' ? 'right' : ''};
  `
  ...
  return (
    <>
      ...
      <ChatLi>
        ...
        <ChatLi>
          <Profile size="42px" imgSrc={DefaultProfile} imgAlt="프로필 이미지"/>
          <ChatDiv className='you'>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quae corrupti alias quidem magnam sunt dicta dignissimos voluptatibus, doloribus unde? Officia nostrum asperiores voluptatum vitae porro nisi minima. Veniam, fugit aspernatur.</ChatDiv>
        </ChatLi>
        <ChatLi>
      ...
      <MessageDiv>
        ...
        <Input className='input_chat-comment' type= 'text' placeholder= '메시지 입력하기...' />
      </MessageDiv>
    </>
  )
};
